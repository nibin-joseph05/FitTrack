...
class _HomeScreenState extends ConsumerState<HomeScreen>
  ...
  void initState() {
    ...
    _headerFade =
        Tween<double>(begin: 0, end: 1).animate(_headerController);
  ...
  Widget build(BuildContext context) {
    ...
    final attendedDaysThisWeek = attendanceHistoryAsync.maybeWhen(
      data: (history) =>
          history.where((a) => a.attended && AppDateUtils.isThisWeek(a.date)).length,
      orElse: () => 0,
    );
              ...
              slivers: [
                ...
                if (todayWorkouts.isEmpty)
                  SliverToBoxAdapter(
                    child: FadeTransition(
                      opacity: _cardsFade,
                      child: _PersonalityMessageCard(userName: userName),
                    ),
                  )
                else
                  SliverList(
        ...
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  PersonalityMessages.gymReadyMessage(userName),
...
class _MotivationQuoteCard extends StatelessWidget {
  ...
  const _MotivationQuoteCard({required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    final attendedDates = attendanceHistory.maybeWhen(
      data: (list) => Set<String>.from(
        (list as Iterable).cast<dynamic>().where((a) => a.attended == true).map((a) {
        ...
        children: [
          ...
          ClipRRect(
            ...
            child: TweenAnimationBuilder<double>(
              tween: Tween(
                  begin: 0,
                  end: (attendedDays / weeklyGoal).clamp(0.0, 1.0)),
